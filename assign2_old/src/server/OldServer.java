...
public class OldServer implements Runnable {
    ...
    public OldServer(int port, int maxPlayersPerGame, int maxConcurrentGames, List<SocketChannel> connectedClients) throws IOException {
        this.port = port;
    ...
    private void handleGameQueue() {
        if (gameQueue.size() >= maxPlayersPerGame) {
            ...
            Runnable gameInstance = new OldGame(gamePlayers);
